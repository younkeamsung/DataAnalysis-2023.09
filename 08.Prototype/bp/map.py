...
def station():
    if request.method == 'GET':
        ...
        return render_template('map/station_res.html')
...
def cctv_pop():
    if request.method == 'GET':
        ...
        return render_template('map/cctv_pop_form.html', columns=columns, colormaps=colormaps)
    else:
        column = request.form['column']
        colormap = request.form['colormap']
        mu.get_cctv_pop(current_app.static_folder, column, colormap)    # static/img/cctv_pop.html 파일
        return render_template('map/cctv_pop_res.html', column=column, colormap=colormap)